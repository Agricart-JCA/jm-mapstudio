...
// Recebe PDF/imagem → salva no Google Drive → registra na tabela plantas
// ================================================================
...
const MAX_MB    = 20
const FOLDER_ID = '1JPWw_s4-6pTHI8bevZVO7HIjSRAy-PvV'
...
// ── Torna o arquivo público (qualquer um com o link pode ver) ──
...
serve(async (req) => {
  ...
  try {
    ...
    // Lê bytes e valida magic bytes (evita MIME spoofing)
    ...
    if (dbErr) { console.error('[upload-planta] db:', dbErr.message); throw new Error('Erro ao registrar a planta') }
    ...
    const safeMsg = ['Arquivo obrigatório','Título obrigatório','Município obrigatório',
      'Arquivo muito grande','Formato não suportado','Arquivo inválido',
      'Usuário não autenticado','Erro ao registrar a planta'].some(s => msg.startsWith(s))
      ? msg : 'Erro interno ao processar upload'
